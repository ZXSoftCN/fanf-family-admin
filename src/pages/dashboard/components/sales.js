...
function Sales ({ data }) {
  return (
    <div className={styles.sales}>
      <div className={styles.title}>月度业务进展图表</div>
            ...
            content={(prop) => {
              ...
              return (<ul className={classnames({ [styles.legend]: true, clearfix: true })}>
                {payload.map((item, key) => {
                  const {payload: innerProp} = item
                  return <li key={key}><span className={styles.radiusdot} style={{ background: item.color }} />{item.value}</li>
                })}
              </ul>)
            }}
          />
          ...
          <Tooltip
            ...
            content={(content) => {
              const list = content.payload.map((item, key) => {
                // const {payload: innerProp} = item
                //item中的name默认取，数据键名。
                return <li key={key} className={styles.tipitem}><span className={styles.radiusdot} style={{ background: item.color }} />{`${item.name}:${item.value}`}</li>
              })
